...
if ( in_array('database', $argv) ){
	...
	$city_id = $city_count = 0;
	foreach($arr as $city_name => $streetArr):
		$city_count++; $city_id++;
		$db->query(
			"INSERT INTO city (`city_id`, `city_name`)
			 VALUES           ($city_id, \"$city_name\");"
		);
		if( $db->error() ) echo "city: ".$db->error()."\n";
		$street_count = 0;
		
		foreach($streetArr as $street_id => $street_name):
			$street_count++;
			$db->query(
				"INSERT INTO street (`street_name`,  `city_id`)
				 VALUES             (\"$street_name\", $city_id);"
			);
